 ...
 * 
 * @version  $Id: Debug.php 158 2010-09-04 16:49:32Z denis $
 */

class Ik_Debug extends Zend_Debug
{
    ...
    public static function setEnabled($enable)
    {
        if (!self::$_enabled && (!self::$_timer or !self::$_vars)) {
            ...
            self::$_timer->setHeader(array('Time (sec)',
                                           'Total (sec)',
                                           'Memory (Kb)',
                                           'Total (Kb)',
                                           'Comment',
    ...
    public static function debug() {
        ...
        preg_match('/\b\s*Ik_Debug::debug\s*\(\s*(.+)\s*\);\s*/i', $code, $arrMatches);
    ...
    {
        self::$_timer->setLabel(sprintf('Timer (%s sec @  %s Kb)',
